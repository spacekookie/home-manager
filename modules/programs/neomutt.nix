...
let
  ...
  genAccountFile = account: with account;
    let
    ...
    in
      {
        target = accountFileName;
        text = ''
          # Generated by Home Manager.

        '' + concatStringsSep mapAttrsToList setOption (
            genCommonFolderHooks account
            // {
              imap_user="'${userName}'";
              imap_pass="'`${passCmd}`'";
...

{
  ...
  config = mkIf cfg.enable {
    ...
    home.file = map (account: {
      target = genAccountFileName account.name;
      text = genAccountConfig account;
    }) neomuttAccounts;
